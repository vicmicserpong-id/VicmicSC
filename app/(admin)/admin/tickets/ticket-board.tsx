      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {BOARD_COLUMNS.map((status) => {
          ...
          return (
            <div
              key={status}
              className="flex w-56 shrink-0 flex-col gap-1.5 rounded-xl bg-muted/40 p-2"
            >
              <div className="flex items-center gap-1.5 px-1">
                <span className={cn("size-2 shrink-0 rounded-full", DOT_COLOR[status])} />
                <h2 className="truncate text-xs font-semibold">{TICKET_STATUS_LABEL[status]}</h2>
                <span className="ml-auto shrink-0 text-xs text-muted-foreground">{items.length}</span>
              </div>

              <div className="flex max-h-[62vh] flex-col gap-1.5 overflow-y-auto overflow-x-hidden">
                {items.length === 0 ? (
                      ...
                      >
                        <div className="flex min-w-0 items-center gap-1">
                          ...
                          <WarrantyBadge
                            ...
                            className="h-4 shrink-0 border-0 px-1 text-[10px]"
                          />
